...
export default function AdminDashboard() {
  ...
  async function sendSummary() {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setTelegramMsg(data.orderCount === 0 ? "No orders found — empty summary sent." : `${t("summary_sent")} (${data.orderCount} orders)`);
      } else {
  ...
  async function changePassword(e: React.FormEvent) {
    ...
    if (newPwd.length < 4) { setPwdMsg({ text: "Password must be at least 4 characters.", error: true }); return; }
  ...
  if (!adminPwd) {
    return <div className="min-h-screen bg-green-50 flex items-center justify-center"><p className="text-gray-400">Loading...</p></div>;
  }

  return (
    <>
      <Head>
        <title>Admin Dashboard — NatureFarm</title>
      ...
      <main className="min-h-screen bg-green-50 pb-8">
        ...
        <div className="bg-green-700 text-white px-4 py-3 flex items-center justify-between sticky top-0 z-10 shadow-md">
          ...
          <button onClick={logout} className="text-green-200 hover:text-white text-sm font-medium flex items-center gap-1">
            🚪 {t("logout")}
          </button>
        </div>
            ...
            {(["products", "orders", "settings"] as Tab[]).map((tb) => (
              <button
                ...
                className={`flex-1 py-3 text-sm font-medium border-b-2 transition-all duration-150 ${
                  tab === tb ? "border-green-600 text-green-700" : "border-transparent text-gray-500 hover:text-gray-700"
                }`}
              >
          ...
          {tab === "products" && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={openAddProduct} className="bg-green-600 text-white text-sm font-medium px-4 py-2 rounded-xl hover:bg-green-700 active:scale-95 transition-all">
                  + {t("add_product")}
                </button>
              </div>
              ...
              {showProductForm && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center p-4 sm:items-center">
                  <div className="bg-white rounded-2xl w-full max-w-md p-5 space-y-4 shadow-2xl">
                    <div className="flex items-center justify-between">
                      <h3 className="font-bold text-lg">{editingProduct ? t("edit_product") : t("add_product")}</h3>
                      <button onClick={() => setShowProductForm(false)} className="text-gray-400 hover:text-gray-600 text-xl">✕</button>
                    </div>

                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t("product_name")} <span className="text-red-500">*</span></label>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t("product_name_hi")}</label>
                    ...
                    <div className="flex gap-3">
                      <div className="flex-1">
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t("display_order")}</label>
                      ...
                      <div className="flex-1 flex flex-col">
                        ...
                        <button type="button" onClick={() => setProductForm({ ...productForm, active: !productForm.active })}
                          className={`flex-1 rounded-xl font-medium text-sm border-2 transition-all ${productForm.active ? "bg-green-50 border-green-400 text-green-700" : "bg-gray-50 border-gray-300 text-gray-500"}`}>
                    ...
                    <div className="flex gap-3 pt-1">
                      ...
                      <button onClick={saveProduct} disabled={productSaving} className="btn-primary py-2.5">{productSaving ? t("saving") : t("save")}</button>
                    </div>
                  </div>
                </div>
              )}
              ...
              {/* Products List */}
              {loadingProducts ? (
                <div className="text-center py-10 text-gray-400"><p className="text-3xl mb-2">⏳</p><p>Loading...</p></div>
              ) : products.length === 0 ? (
                <div className="card text-center py-8 text-gray-400"><p className="text-3xl mb-2">📭</p><p>{t("no_products")}</p></div>
              ) : (
                <>
                    ...
                    return (
                      <div key={cat}>
                        <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-2">
                          ...
                          <span className="text-gray-300 font-normal normal-case tracking-normal">({catProducts.length})</span>
                        </h3>
                        <div className="space-y-2">
                          {catProducts.map((product) => (
                            <div key={product.id} className={`card flex items-center gap-3 py-3 ${!product.active ? "opacity-60" : ""}`}>
                              <div className="flex-1 min-w-0">
                                <p className="font-semibold text-gray-800 truncate">{locale === "hi" && product.nameHi ? product.nameHi : product.name}</p>
                                <p className="text-xs text-gray-500">
                                  {product.unit} · {product.active ? <span className="text-green-600 font-medium">{t("active")}</span> : <span className="text-gray-400">{t("inactive")}</span>}
                                </p>
                              </div>
                              <button onClick={() => openEditProduct(product)} className="text-blue-500 hover:text-blue-700 text-sm font-medium px-2 py-1">✏️</button>
                              <button onClick={() => toggleProductActive(product)} disabled={togglingId === product.id}
                                className={`text-sm font-medium px-3 py-1.5 rounded-lg border transition-all ${product.active ? "border-red-200 text-red-500 hover:bg-red-50" : "border-green-200 text-green-600 hover:bg-green-50"}`}>
            ...
            <div className="space-y-4">
              <div className="card space-y-3">
                <div className="flex items-center gap-3">
                  <div className="flex-1">
                    <label className="text-sm font-medium text-gray-700 block mb-1">{t("orders_for")}:</label>
                    <input type="date" className="input-field" value={ordersDate} onChange={(e) => setOrdersDate(e.target.value)} />
                  </div>
                  <button onClick={loadOrders} className="bg-green-600 text-white px-4 py-3 rounded-xl text-sm font-medium hover:bg-green-700 active:scale-95 transition-all mt-5">🔄</button>
                </div>
                ...
                <button onClick={sendSummary} disabled={sendingTelegram}
                  ...
                  {sendingTelegram ? <><span>⏳</span> {t("sending")}</> : <><span>📨</span> {t("send_summary")}</>}
                </button>

                {telegramMsg && (
                  <p className={`text-sm text-center px-3 py-2 rounded-lg ${telegramMsg.includes("sent") || telegramMsg.includes("भेजा") ? "bg-green-50 text-green-700" : "bg-red-50 text-red-600"}`}>
                    {telegramMsg}
                  </p>
                )}
              </div>
              ...
              {loadingOrders ? (
                <div className="text-center py-10 text-gray-400"><p className="text-3xl mb-2">⏳</p><p>Loading orders...</p></div>
              ) : orders.length === 0 ? (
                <div className="card text-center py-8 text-gray-400"><p className="text-3xl mb-2">📭</p><p>{t("no_orders")}</p></div>
              ) : (
                <>
                  <div className="flex items-center justify-between">
                    <h2 className="section-title mb-0">{t("total_orders")}: <span className="text-green-700">{orders.length}</span></h2>
                  </div>
                    ...
                    {orders.map((order, idx) => (
                      <div key={order.orderId} className="card">
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <span className="text-xs text-gray-400 font-mono">#{order.orderId.slice(-6)}</span>
                        </div>
            ...
            <div className="space-y-5">
              <div className="card space-y-4">
                ...
                <form onSubmit={changePassword} className="space-y-3">
                  <div>
                    ...
                    <input type="password" className="input-field" value={currentPwd} onChange={(e) => { setCurrentPwd(e.target.value); setPwdMsg({ text: "", error: false }); }} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">{t("new_password")}</label>
                    <input type="password" className="input-field" value={newPwd} onChange={(e) => { setNewPwd(e.target.value); setPwdMsg({ text: "", error: false }); }} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">{t("confirm_password")}</label>
                    <input type="password" className="input-field" value={confirmPwd} onChange={(e) => { setConfirmPwd(e.target.value); setPwdMsg({ text: "", error: false }); }} />
                  </div>
                  {pwdMsg.text && (
                    <p className={`text-sm px-3 py-2 rounded-lg ${pwdMsg.error ? "bg-red-50 text-red-600" : "bg-green-50 text-green-700"}`}>{pwdMsg.text}</p>
                  )}
                  <button type="submit" disabled={pwdSaving} className="btn-primary">{pwdSaving ? t("saving") : t("update_password")}</button>
                </form>
              </div>
              ...
              <div className="card bg-green-50 border-green-100">
                ...
                <div className="space-y-2 text-sm text-gray-600">
                  <div className="flex justify-between"><span>App</span><span className="font-medium text-green-700">NatureFarm v1.0</span></div>
                  <div className="flex justify-between"><span>Auto-summary</span><span className="font-medium text-green-700">Daily 5:00 AM IST</span></div>
                  <div className="flex justify-between"><span>Telegram</span><span className="font-medium text-green-700">Connected ✅</span></div>
                </div>
              </div>
            </div>
          )}
